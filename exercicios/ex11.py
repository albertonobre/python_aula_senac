# Faça um algoritmo que receba o preço de custo de um produto e mostre o valor de venda. Sabe-se que o preço de
# custo receberá um acréscimo de acordo com um percentual informado pelo usuário.

custo = float(input("Digite o preço de custo do produto:"))
percentual = float(input("Digite o percentual de ganho desejado na venda do produto:"))

ganho = percentual/100
venda = 1 + ganho

print("O valor de venda do produto será de:", custo * venda)
